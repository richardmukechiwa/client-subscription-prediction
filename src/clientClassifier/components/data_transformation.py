...
class DataTransformation:
    ...
    def initiate_data_transformation(self):
        ...
        logger.info(
            f"4. education: Level of education (e.g., primary, secondary, tertiary)"
        )
        ...
        logger.info(
            f"9. contact: Type of communication used to contact the client (e.g., cellular, telephone)"
        )
        ...
        logger.info(
            f"13. campaign: Number of contacts performed during this campaign for this client"
        )
        logger.info(
            f"14. pdays: Number of days since the client was last contacted from a previous campaign (999 means client was not previously contacted)"
        )
        logger.info(
            f"15. previous: Number of contacts performed before this campaign for this client"
        )
        logger.info(
            f"16. poutcome: Outcome of the previous marketing campaign (e.g., success, failure)"
        )
        logger.info(
            f"17. y: Target variable (whether the client subscribed to a term deposit or not)"
        )
        ...
        logger.info(
            "Most features in the dataset show low pairwise correlation, suggesting minimal multicollinearity."
        )
        logger.info(
            "This is good for tree-based models which are not sensitive to multicollinearity, but may still affect linear models like logistic regression."
        )
        print()
        print("<" * 70)
        print()
        # Target correlation
        logger.info(
            "'duration' has a very strong positive correlation with the target variable 'y'."
        )
        logger.info(
            "However, 'duration' is a data leakage feature — it’s only known after the campaign call and directly impacts the prediction."
        )
        logger.warning(
            "Including 'duration' would artificially inflate model performance. Exclude it during training to ensure real-world generalization."
        )
        print()
        print("<" * 70)
        print()
        logger.info(
            "Consider removing 'duration' from the feature set to avoid data leakage."
        )
        logger.info(
            "Features like 'previous' and 'pdays' also show some correlation with the target variable but"
        )
        ...
        logger.info(
            "Consider conducting feature importance analysis using Random Forest or XGBoost to refine feature selection."
        )
        print()
        print("<" * 70)
        print()
